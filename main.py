...
class Main(object):
    ...
    def load_data(self):
        ...
        for split in ['train', 'test', 'valid', '1-1_test', '1-n_test', 'n-1_test', 'n-n_test']:
        ...
        for split in ['test', 'valid', '1-1_test', '1-n_test', 'n-1_test', 'n-n_test']:
        ...
        self.data_iter = {
            ...
            '1-1_test_head': get_data_loader(TestDataset, '1-1_test_head', self.p.batch_size),
            '1-1_test_tail': get_data_loader(TestDataset, '1-1_test_tail', self.p.batch_size),
            '1-n_test_head': get_data_loader(TestDataset, '1-n_test_head', self.p.batch_size),
            '1-n_test_tail': get_data_loader(TestDataset, '1-n_test_tail', self.p.batch_size),
            'n-1_test_head': get_data_loader(TestDataset, 'n-1_test_head', self.p.batch_size),
            'n-1_test_tail': get_data_loader(TestDataset, 'n-1_test_tail', self.p.batch_size),
            'n-n_test_head': get_data_loader(TestDataset, 'n-n_test_head', self.p.batch_size),
    ...
    def evaluate(self, split, epoch=0):
        ...
        self.logger.info \
            ('[Epoch {} {}]: MRR: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(epoch, split, results['left_mrr'], results['right_mrr'], results['mrr']))
        self.logger.info \
            ('[Epoch {} {}]: MR: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(epoch, split, results['left_mr'],
                                                                                  results['right_mr'], results['mr']))
        self.logger.info \
            ('[Epoch {} {}]: HIT@1: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(epoch, split, results['left_hits@1'],
                                                                                  results['right_hits@1'], results['hits@1']))
        self.logger.info \
            ('[Epoch {} {}]: HIT@10: Tail : {:.5}, Head : {:.5}, Avg : {:.5}'.format(epoch, split, results['left_hits@10'],
    ...
    def run_epoch(self, epoch):
        ...
        for step, batch in enumerate(train_iter):
            ...
            if step % 100 == 0:
                self.logger.info \
                    ('[E:{}| {}]: Train Loss:{:.5},  Val MRR:{:.5}, Valid MR: {: }, Valid HIT@10: {:.5}, Valid HIT@1: {:.5} \t{}'.format(epoch, step, np.mean(losses), self.best_val_mrr, self.best_val_mr, self.best_val_hit10, self.best_val_hit1, self.p.name))
    ...
    def fit(self):
        ...
        for epoch in range(self.p.max_epochs):
            ...
            if val_results['hits@1'] > self.best_val_hit1:
                self.best_val		= val_results
                self.best_val_mrr	= val_results['mrr']
                self.best_val_hit1  = val_results['hits@1']
                self.best_val_hit10 = val_results['hits@10']
                self.best_val_mr    = val_results['mr']
                self.best_epoch		= epoch
                self.save_model(save_path)
            self.logger.info('[Epoch {}]:  Training Loss: {:.5},  Valid MRR: {:.5}, Valid MR: {: }, Valid HIT@10: {:.5}, Valid HIT@1: {:.5}, \n\n\n'.format(epoch, train_loss, self.best_val_mrr, self.best_val_mr, self.best_val_hit10, self.best_val_hit1))
            with open(self.p.name + '_log.txt', 'a') as f:
                f.write('Epoch: {} Valid MRR: {:.5} Valid HIT@1: {:.5}'.format(epoch, self.best_val_mrr,
                                                                               self.best_val_hit1) + '\n')
